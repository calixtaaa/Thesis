...
- **Main screen:** Scrollable product grid, pill-shaped search bar (“Search for products”) with magnifying glass and backspace icons, navbar (hamburger, theme toggle), footer with **SyntaxError™** and version (v.0.03), live Philippine date/time (badge), and buttons: Reload (RFID), Buy RFID Card, Report, How to use?, Patch Notes.
...
- **Bug Report:** Added in-app Report screen (category + explanation + submit) saved locally to `bug_reports/bug_reports.ndjson`.
